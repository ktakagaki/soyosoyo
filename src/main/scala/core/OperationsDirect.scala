...
final trait OperationsDirect {
   ...
   */
  def add[@specialized(Int, Double, Float, Long) T]( data1: Array[T],
                                                     data2: Array[T] ): Array[T] =
    data1.synchronized data2.synchronized {

    var c = 0
    val tempReturn = new Array[T](data1.length)
    while(c < data2.length){ //By using data2.length here, ensures (data1.length == data2.length) is necessary to run correctly
  ...
  def addTo[@specialized(Int, Double, Float, Long) T]( data1: Array[T],
                                                       data2: Array[T] ): Unit =
    data1.synchronized data2.synchronized {
